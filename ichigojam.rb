...
class Std15
  ...
  def initialize(screen_w, screen_h, buff_w, buff_h)
    @buff_w = buff_w
    @buff_h = buff_h
    @dot_w = screen_w / buff_w / CHAR_W
    @dot_h = screen_h / buff_h / CHAR_H
    @buff = Array.new(buff_w * buff_h)
    @cursor_x = 0
    @cursor_y = 0
    cls()
  end
  ...
  def draw_char (cx, cy, c)
    glyph = ICHIGOJAM_FONT[c]
    for y in 0..CHAR_H-1
      line = (glyph >> ((CHAR_H-y-1)*CHAR_W)) & 0xff
      for x in 0..CHAR_W-1
	if ((line >> (CHAR_W-x-1)) & 0x1) == 0x1 then
	  x0 = (cx * CHAR_W + x) * @dot_w
	  y0 = (cy * CHAR_H + y) * @dot_h
          Gosu.draw_rect(x0, y0, @dot_w, @dot_h, Gosu::Color::WHITE)
	end
      end
    end
  end
